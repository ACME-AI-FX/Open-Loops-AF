...
try:
    ...
    # build: a row the agent could not place (cell full -> empty item_id) stays pending, not "posted"
    ...
    roadmap.save(d)
    roadmap.agent.run = lambda *a, **k: _P('<<<ROADMAP>>>{"created": [{"id": "r1", "item_id": "m1", "url": "", "note": ""}, '
                                           '{"id": "r2", "item_id": "", "url": "", "note": "cell full"}]}<<<END>>>')
    try:
    ...
    check(rows["r1"]["posted_id"] == "m1" and rows["r2"]["posted_id"] is None, "created row is posted; uncreated row stays pending")
